...
public record FavoriteDetailResponse(
    ...
    public record PlaceImages(
        ...
        public static PlaceImages of(PlaceImage image) {
            return new PlaceImages(image.getId(), image.getUrl());
        }
    }

    public static FavoriteDetailResponse of(Favorite favorite) {
        return new FavoriteDetailResponse(
            favorite.getId(),
            ...
            favorite.getPlace().getLatitude(),
            favorite.getPlace().getLongitude(),
            ...
            favorite.getPlace().getBusinessStartTime().format(DateTimeFormatter.ofPattern("HH:mm")),
            favorite.getPlace().getBusinessClosingTime().format(DateTimeFormatter.ofPattern("HH:mm")),
            favorite.getPlace().getDayOff(),
            favorite.getPlace().getImages().stream().map(PlaceImages::of).toList()
        );
    }
}
